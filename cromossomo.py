...
from sklearn.cross_validation import train_test_split as cv_train
...
class Cromossomo():
    ...
    def __init__(self, ind):
        self.inicializarVariaveis(ind)

    #######################################
    # INICIALIZADORES

    def inicializarVariaveis(self, ind):
        self.individuo = ind
    ...
    def calcularFitness(self):
        ...
        scores = cross_validation.cross_val_score(clf, dataset.data, dataset.target, cv=10, scoring='accuracy')
        self.acuracia = round(100*scores.mean(), 2)

        # X_train, X_test, Y_train, Y_test = cv_train(dataset.data, dataset.target, test_size=0.3, random_state=42)
